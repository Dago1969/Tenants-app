...
export class ProjectsCrudComponent implements OnInit {
  ...
  private loadInitialData(): void {
    ...
    tenantRequest.pipe(
      ...
      next: ({ tenant, administrators, project }) => {
        ...
        this.autoGeneratedFooter = this.project.footer ?? '';
        this.loading = false;
      },
      error: () => {
  ...
  private createProjectFromTenant(tenant: TenantAppPointerDto | null, clientCode: string): ProjectDto {
    ...
    project.footer = this.buildDefaultProjectFooter();
    return project;
  }
  ...
  onFooterSourceChange(): void {
    this.syncGeneratedFooter();
  }

  private syncGeneratedFooter(): void {
    const nextFooter = this.buildDefaultProjectFooter();

    if (!this.project.footer || this.project.footer === this.autoGeneratedFooter) {
      this.project.footer = nextFooter;
    }
